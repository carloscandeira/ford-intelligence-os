...
# ─────────────────────────────────────────────────────────────
# PRICE TARGETS — preço de referência via Tabela FIPE (scraper/fipe_prices.py)
# Só precisa de marca + nome do modelo; a FIPE resolve as versões do lineup.
# ─────────────────────────────────────────────────────────────
...
PRICE_TARGETS = [
    ...
    {"marca": "Mitsubishi",  "modelo": "Triton"},
]
...
# ─────────────────────────────────────────────────────────────
# ETAPA 3 — Preços de referência (Tabela FIPE) → ver scraper/fipe_prices.py
# (webmotors.com.br foi abandonado: bloqueia scraping com 403)
# ─────────────────────────────────────────────────────────────
...
async def scrape_all():
    ...
    async with async_playwright() as p:
        ...
        # ── ETAPA 3: Preços de referência (Tabela FIPE) ──────
        # webmotors.com.br bloqueia scraping (403). A FIPE é a referência
        # oficial de preço no Brasil: pública, gratuita e citável.
        print("\n" + "="*55)
        print("ETAPA 3 — Precos de referencia (Tabela FIPE)")
        print("="*55)

        from scraper.fipe_prices import fetch_fipe_prices

        for target in PRICE_TARGETS:
            marca, modelo = target["marca"], target["modelo"]
            print(f"  💰 {marca} {modelo} — FIPE")
            rows, errors = fetch_fipe_prices(marca, modelo)
            for r in rows:
                all_specs.append(ExtractedSpec(
                    marca=r["marca"], modelo=r["modelo"], versao=r["versao"],
                    campo=r["campo"], valor=r["valor"], unidade=r["unidade"],
                    fonte_url=r["fonte_url"],
                ))
            all_errors.extend(errors)
            if rows:
                print(f"     ✅ {len(rows)} precos FIPE ({rows[0].get('mes_referencia', '')})")
                for r in rows:
                    print(f"        {r['versao'][:42]}: R$ {int(r['valor']):,}".replace(",", "."))
            else:
                print(f"     ⚠ 0 precos FIPE")
